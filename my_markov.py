...
def make_text(chains, capital_words=True, new_lines=False):
    ...
    while True:
        try:
            ...
            if check_characters(starting_n_gram, total):
        ...
        if not check_characters([starting_n_gram[0]], total):
            break
        else:
...
def check_characters(n_gram, total):
    """Returns true if the next n_gram can be added to the final list of words
    without going over the twitter character limit.
    """

    characters = sum([len(word) for word in n_gram])
    if total + characters + len(n_gram) > 140:
        return False
    else:
        return True
